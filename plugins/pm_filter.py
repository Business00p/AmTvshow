...
async def next_page(bot, query):
    ...
    btn.insert(0,[
	BotCommand("💫 𝑹𝒆𝒎𝒐𝒗𝒆 𝑨𝒅𝒔", callback_data="/plan"),
        ])
    btn.insert(1, [
    ...
    btn.insert(2, [
        ...
        BotCommand("📈 Mᴏsᴛ Sᴇᴀʀᴄʜ", callback_data="/mostlist")
])
    ...
    if n_offset == 0:
        ...
        btn.append(
            [InlineKeyboardButton("⏪  ʙᴀᴄᴋ", callback_data=f"next_{req}_{key}_{off_set}"),
             InlineKeyboardButton(f"🗓️ ᴘᴀɢᴇ {math.ceil(int(offset) / int(MAX_BTN)) + 1} / {math.ceil(total / int(MAX_BTN))}", callback_data="pages")]
        )
    elif off_set is None:
        btn.append(
            [InlineKeyboardButton(f"🗓️ {math.ceil(int(offset) / int(MAX_BTN)) + 1} / {math.ceil(total / int(MAX_BTN))}", callback_data="pages"),
    ...
    if n_offset== '':
        ...
        btn.append(
            [InlineKeyboardButton(" ⏪  ʙᴀᴄᴋ", callback_data=f"season_search#{season}#{key}#{offset- int(MAX_BTN)}#{orginal_offset}#{req}"),
             InlineKeyboardButton(f"🗓️ {math.ceil(offset / int(MAX_BTN)) + 1}/{math.ceil(total / int(MAX_BTN))}", callback_data="pages",),
            ])
    elif offset==0:
        btn.append(
            [InlineKeyboardButton(f"🗓️ {math.ceil(offset / int(MAX_BTN)) + 1}/{math.ceil(total / int(MAX_BTN))}",callback_data="pages",),
...
async def auto_filter(client, msg, spoll=False , pm_mode = False):
    if not spoll:
        ...
        if not files:
            if settings["spell_check"]:
                ai_sts = await msg.reply_text(f'Ai is Cheking For Your Spelling..Please Wait.')
    ...
    if offset != "":
        if total_results >= MAX_BTN:
            ...
            btn.insert(0,[
                InlineKeyboardButton("♻️𝗦𝗲𝗻𝗱 𝗔𝗹𝗹 𝗙𝗶𝗹𝗲𝘀", callback_data=batch_link),
                InlineKeyboardButton("🌍 ʟᴀɴɢᴜᴀɢᴇ", callback_data=f"languages#{key}#{offset}#{req}")
            ])
            btn.insert(1,[
        ...
        btn.insert(0,[
            InlineKeyboardButton("♻️ 𝗦𝗲𝗻𝗱 𝗔𝗹𝗹 𝗙𝗶𝗹𝗲𝘀", callback_data=batch_link),
            ])
    ...
    if offset != "":
        ...
        req = message.from_user.id if message.from_user else 0
        btn.append(
            [InlineKeyboardButton(text=f"🗓️ 1/{math.ceil(int(total_results) / int(MAX_BTN))}", callback_data="pages"),
